...
class LoRAQKVLinear(nn.Module, LoRALinear):
    # LoRA implemented in a dense layer
    def __init__(
        self,
        # ↓ this part is for pretrained weights
        ...
        **kwargs,
        ...
        if r > 0 and any(enable_lora):
            ...
            self.lora_B = nn.Parameter(torch.zeros(out_features // len(enable_lora) * sum(enable_lora), r))  # (256, 2))
            ...
            if enable_k:
                self.lora_ind.extend(range(self.linear.in_features, self.linear.in_features + self.linear.in_features))
            if enable_v:
                self.lora_ind.extend(range(self.linear.in_features + self.linear.in_features, self.linear.out_features))
            self.reset_parameters()
    ...
    def zero_pad(self, x: torch.Tensor) -> torch.Tensor:
        ...
        result[:, self.lora_ind] = x.reshape(
            -1, self.linear.out_features // len(self.enable_lora) * sum(self.enable_lora)
        )  # (4096, 256)

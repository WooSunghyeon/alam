...
torch::Tensor calc_precision(torch::Tensor b, torch::Tensor C, torch::Tensor w, double target, double aq_bit) {
    TORCH_CHECK(b.device().is_cpu(), "b must be a CPU tensor!");
    TORCH_CHECK(b.is_contiguous(), "b must be contiguous!");
    TORCH_CHECK(C.device().is_cpu(), "C must be a CPU tensor!");
    TORCH_CHECK(C.is_contiguous(), "C must be contiguous!");
    ...
    while (b_sum > target) {        // Pick up the smallest increment (largest decrement)
        ...
        q.pop();
        //auto delta_b = b_data[i] - next_b[b_data[i]];
        if (b_data[i]==2) {delta_b = b_data[i] - aq_bit;}
        else {delta_b = b_data[i] - next_b[b_data[i]];}
...
torch::Tensor zero_calc_precision(torch::Tensor b, torch::Tensor C, torch::Tensor w, double target, bool zeroact) {
    TORCH_CHECK(b.device().is_cpu(), "b must be a CPU tensor!");
    TORCH_CHECK(b.is_contiguous(), "b must be contiguous!");
    TORCH_CHECK(C.device().is_cpu(), "C must be a CPU tensor!");
    TORCH_CHECK(C.is_contiguous(), "C must be contiguous!");
    ...
    double b_sum = 0;
    double state = 0;
    for (int64_t i = 0; i < N; i++) if (b_data[i] > 0) {
        auto delta_b = b_data[i] - next_b[b_data[i]];
        auto delta = get_obj(C_data[i], b_data[i]) / (w_data[i] * delta_b);
        q.push(std::make_pair(delta, i));
        b_sum += b_data[i] * w_data[i];
    ...
    while (b_sum > target) {        // Pick up the smallest increment (largest decrement)
        ...
        b_data[i] = next_b[b_data[i]];
        b_sum -= w_data[i] * delta_b;
        if (zeroact){state=0;} else {state=1;}
        if (b_data[i] > state) {
            auto delta_b = b_data[i] - next_b[b_data[i]];
            auto delta = get_obj(C_data[i], b_data[i]) / (w_data[i] * delta_b);
            q.push(std::make_pair(delta, i));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("zero_calc_precision", &calc_precision, "zero_calc_precision");

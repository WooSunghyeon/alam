...
class AutoPrecision:
    def __init__(self, model, quantizer, bits, max_bits,
                 ...
                 momentum=0.99, warmup_iter=100, debug=False, save_bit_path=None, load_bit_path=None):
        self.model = model
        ...
        # for bit saving & loading
        self.save_bit_path=save_bit_path
        self.load_bit_path=load_bit_path
    ...
    def iterate(self, backprop):
        ...
        if self.load_bit_path == None:    
            if self.iter == 0 or self.iter == self.adapt_step :
                ...
                for i, l in enumerate(self.quantizer.bits):
    ...
    def refresh_bits(self):
        ...
        if self.load_bit_path == None:
            ...
            bits_tensor = ext_calc_precision.calc_precision(bits_tensor,
                                                            ...
                                                            self.total_bits,
                                                            config.aq_bit)
            if self.save_bit_path != None:
                torch.save(bits_tensor, self.save_bit_path)
        else:
            bits_tensor = torch.load(self.load_bit_path)
